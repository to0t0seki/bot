...
from cex_order_book_checker import CexOrderBookChecker, calculate_buy_price, calculate_sell_price,fetch_orderbook_bybit_OAS
import sys
...
def trade1():
    ...
    oas_asks_orderbook = CexOrderBookChecker('OASUSDT').fetch_orderbook('asks')
    ...
    geek_bids_orderbook = CexOrderBookChecker('GEEKUSDT').fetch_orderbook('bids')
...
def trade2(buy_price:float=None):
    ...
    if buy_price is None:
        geek_asks_orderbook = CexOrderBookChecker('GEEKUSDT').fetch_orderbook('asks')
...
def check_oas_best_price(oas_amount:float=None):
    print(f'bybitでの計算')
    oas_bids_orderbook = fetch_orderbook_bybit_OAS()
    sell_amount, sell_price = calculate_sell_price(oas_bids_orderbook, oas_amount)
